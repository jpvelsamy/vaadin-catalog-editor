...
public class ThreeCardContainer extends HorizontalLayout {
	...
	private CuteCatalogItem firstItem;
	private CuteCatalogItem secondItem;
	private CuteCatalogItem thirdItem;
	...
	public ThreeCardContainer(Collection<CatalogueItem> itemCollection) {
		...
		itemCollection.forEach(item -> {
			...
			CuteCatalogItem catalogComponent = new CuteCatalogItem(item, index);
	...
	public void addFirstCard(CuteCatalogItem newItem) {
	...
	public void addCard(CuteCatalogItem newItem) {
	 ...
	 */
	public void removeCard(CuteCatalogItem removeCandidate) {
	...
	public void updateView(final ImmutableTriple<Integer, Integer, Integer> indexTuple) {
		...
		if (firstItem == null) {
			...
			if (this.itemMap.containsKey(firstItemIndex)) {
				CuteCatalogItem nthItem = this.itemMap.get(firstItemIndex);
				if (nthItem != null) {
				...
				if (this.itemMap.containsKey(secondItemIndex)) {
					CuteCatalogItem nplusOnethItem = this.itemMap.get(secondItemIndex);
					...
					if (this.itemMap.containsKey(thirdItemIndex)) {
						CuteCatalogItem nplusTwothItem = this.itemMap.get(thirdItemIndex);
